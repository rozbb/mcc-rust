...
fn decrypt_with_factorization(ciphertext: &Int, p: &Int, q: &Int, exponent: &Int) -> Int {
    // Totient ϕ(pq) = (p-1)(q-1) for p, q prime
    let et = (p - Int::one()) * (q - Int::one());
    // Decryption exponent
    let d = inv_mod(exponent, &et).expect("No mod inverse!");
    let n = p * q;
    mod_exp(ciphertext, &d, &n)
}

fn decrypt_with_CRT(ciphertexts: &Vec<Int>, moduli: &Vec<Int>, exponent: &Int) -> Int {
    ...
    let root = find_cube_root(&result);
    root
}

fn crack_message(ciphertexts: &Vec<Int>, moduli: &Vec<Int>, exponent: &Int) -> Int {
...
fn tst40() {
    ...
    for n_primes in &[n_primes_1, n_primes_2] {
        let mut primes = rand::sample(&mut rng, PRIMES, *n_primes)
                               .iter()
        ...
        let ciphertexts = moduli.iter()
                                .map(|n| mod_exp(&message, &e, n))
                                .collect::<Vec<Int>>();
        let cracked = crack_message(&ciphertexts, &moduli, &e);
